...
It is an error for a client to send the server a message with any _message type_ value that is **not listed below with "Client->Server"**.
...
It is an error for a client that has not identified itself as a _camera_ (see `IAmCamera` below) to send a `Plate` message.
...
It is **never acceptable** to ticket a car that had an average speed _below_ the speed limit.
